...
class UnattendedObjectDetector:
    def __init__(self):
        ...
        self.detections = {}  # Format: {object_id: {"time": float, "name": str, "location": str, "alerted": bool}}
        ...
        while True:
            ...
            for i in range(len(boxes)):
                if i in indexes:
                    ...
                    # Draw bounding box and basic label
                    label = f"{object_name} ({elapsed_time:.1f}s)"
                    cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                    cv2.putText(frame, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 
                              0.5, color, 2)

                    # Check and flag unattended objects
                    if elapsed_time >= self.unattended_threshold:
                        ...
                        # Trigger alerts if not already alerted
                        if not self.detections[object_id]["alerted"]:
                            # Beep alert
                            winsound.Beep(1000, 500)  # 1000 Hz for 500 ms
                            # Send email
                            self.send_email_alert(object_name, self.detections[object_id]["location"])
                            self.detections[object_id]["alerted"] = True
